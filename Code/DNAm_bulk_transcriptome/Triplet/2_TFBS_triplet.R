...
setwd("/home/yang/PM_AD/Submission/Table/Supplementary_table3")
...
for (i in 1:nrow(data)) {
  ...
  # Filter the GFF3 data to find regions that include positions 12-13
  filtered_df <- df[sapply(1:nrow(df), function(i) {
    start <- df$start[i]
    end <- df$end[i]
    
    # Check if the region includes positions 12 and 13 (regulatory CpG)
    (start <= 13 && end >= 12)
  }), ]
...
# write.csv(data, 'Triplet_results_TFBS.txt')
