...
setwd("/home/yang/PM_AD/3_Triplet/")
...
# Read data
Pla_PM1AD1_DMR <- read.csv("/home/yang/PM_AD/2_DMC_DEG/Placenta_PM1AD0_vs_PM1AD1_DMR_with_covariates.txt", row.names = 1)
...
# Create triplet data 
Pla_triplet.promoter.jasper <- create_triplet_distance_based(
  ...
  motif.search.window.size = 24,
  motif.search.p.cutoff = 5e-02,
  cores = 1
)

Pla_triplet.promoter.remap <- create_triplet_distance_based(
  region = Pla_PM1AD1_DMR.se,
  target.method = "genes.promoter.overlap",
  genome = "hg38",
...
Pla_triplet.distal.window.jasper <- create_triplet_distance_based(
  region = Pla_PM1AD1_DMR.se,
  genome = "hg38", 
...
Pla_triplet.distal.window.remap <- create_triplet_distance_based(
  region = Pla_PM1AD1_DMR.se,
  genome = "hg38", 
  target.method = "window",
  target.window.size = 500 * 10^3,
...
# Load regulons for TF activity analysis
regulons.dorothea <- dorothea::dorothea_hs

# Read RNA-seq data (DEG)
PM_pla_genes_wilcox <- read.csv("/home/yang/PM_AD/2_DMC_DEG/Placenta_PM1AD0_vs_PM1AD1_DEG_with_covariates.txt", check.names = FALSE, row.names = 1)
...
Pla_all_genes <- read.csv("/home/yang/PM_AD/2_DMC_DEG/Placenta_transriptome_cov_PM1AD0_PM1AD1.txt", check.names = FALSE, row.names = 1)
...
results.interaction.model <- interaction_model(
  triplet = Pla_triplet.distal.window.jasper, 
...
# write.csv(filtered_data, "/home/yang/PM_AD/Submission/Table/Supplementary_table3/Promoter.txt")
# write.csv(filtered_data, "/home/yang/PM_AD/Submission/Table/Supplementary_table3/Distal.txt")
